...
GST_DEBUG_CATEGORY (appsrc_lpbin_debug);
#define GST_CAT_DEFAULT appsrc_lpbin_debug
 ...
 * from. Since we provided the appsrc:// uri to lpbin, this will be the
...
main (int argc, char *argv[])
{
  ...
  GST_DEBUG_CATEGORY_INIT (appsrc_lpbin_debug, "appsrc-lpbin", 0,
      "appsrc lpbin example");
  ...
  bus = gst_pipeline_get_bus (GST_PIPELINE (app->lpbin));

  /* add watch for messages */
  gst_bus_add_watch (bus, (GstBusFunc) bus_message, app);

  /* set to read from appsrc */
  g_object_set (app->lpbin, "uri", "appsrc://", NULL);
  ...
  gst_element_set_state (app->lpbin, GST_STATE_PLAYING);
  ...
  gst_element_set_state (app->lpbin, GST_STATE_NULL);
