...
static GstPad * gst_fc_bin_request_new_pad (GstElement *element, GstPadTemplate *templ, const gchar* name, const GstCaps *caps);
...
gst_fc_bin_class_init (GstFCBinClass * klass)
{
  ...
  gst_element_class_set_static_metadata (element_class, "Flow Controller",
	...
	element_class->request_new_pad = GST_DEBUG_FUNCPTR (gst_fc_bin_request_new_pad);
...
gst_fc_bin_init (GstFCBin * fcbin)
{
  ...
  fcbin->select[GST_FC_BIN_STREAM_TEXT].media_list[1] = "application/x-subtitle";
  ...
  gint i,j;
	...
	if (selected->selector) {
		...
		if ((sinkpad = gst_element_get_request_pad (selected->selector, pad_name))) {
			...
			g_object_set_data (G_OBJECT(ghost_sinkpad), "fcbin.srcpad", selected->srcpad);
...
unknown_type:
	GST_ERROR_OBJECT (fcbin, "unknown type %s for pad %s", name);
